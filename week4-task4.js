...
async function index() {
    ...
    await main.lineaRun(async (addressData) => {
        ...
        for (let access of addressData) {
             ...
             * ************************************************************
             */
            const mint = await isMint(address, access.ProxyObj);
            if (mint === false) {
                let result = null;
                let j = 0;
...
async function isMint(address, proxyObj) {
    // 请求 URL
    const url = `https://public-api.phosphor.xyz/v1/listings/redemption-eligibility?listing_id=d3542d49-273c-4f2d-9d33-8904c773ed14&eth_address=${address}`;
    // 请求头
    const headers = {
        'accept': '*/*',
